...
namespace Soat.Eleven.FastFood.Api.Controllers
{
    ...
    public class PagamentoRestEndpoint : ControllerBase
    {
        ...
        [HttpGet]
        [Authorize]
        public async Task<IActionResult>StatusPagamentoPedido([FromBody] Guid IdPedido)
        {
            try
            {
                var controller = new PedidoController(_pedidoDataSource, _pagamentoDataSource);
                var pedidoCriado = await controller.StatusPagamentoPedido(IdPedido);
                return CreatedAtAction(nameof(pedidoCriado), pedidoCriado);
            }
            catch (Exception ex)
            {
